/**
 * Green pufferfish enemy with three behavior phases:
 * normal swim → transition inflate → bubble (inflated) swim.
 * Includes death animation that floats upward before removal.
 *
 ...
 * @property {boolean} transitionStarted Internal flag to avoid overlapping transitions.
...
class PufferFishGreen extends MovableObject {
   ...
   * Safeguards with an internal flag to avoid double-starting.
   */
   ...
   * for a short time before marking the sprite for removal.
   */
  die() {
    if (this.isDead) return;
    this.isDead = true;
